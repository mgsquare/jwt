...
import (
	...
	"github.com/mgsquare/jwt/database"
	"golang.org/x/crypto/bcrypt"

	helper "github.com/mgsquare/jwt/helpers"
	"github.com/mgsquare/jwt/models"
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while checking checking for the email"})
			return
		}
...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		groupStage := bson.D{
			{"$group", bson.D{
				{"_id", bson.D{{"_id", "null"}}}, {"total_count", bson.D{{"$sum", 1}}}, {"data", bson.D{{"$push", "$$ROOT"}}}}}}
